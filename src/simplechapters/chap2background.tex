...
By using precise timing and simple math, the distance from the transmitter to the target object and back to the receiver can be calculated.
...
times the speed of light over two
(the two accounting for the two-way travel). $\Delta t$ must be very precisely determined,
...
% % TODO explain more about range resolution include a formula
...
Chirps can be made in many ways, a common chirp configuration is a sawtooth shape (Figure \ref{fig:fmcw}).
...
$\Delta f$ in \ref{eq:chirp_range} represents the change in frequency from transmit to receive and $\frac{df}{dt}$
...
\begin{figure}[h]
    ...
    \caption{A series of sawtooth shaped up-chirps, blue representing the transmitted chirp and orange the delayed received signal.
        $\Delta f$ or $\Delta t$ can be used to calculate the range to the target using Eqs. \ref{eq:radar_range}, \ref{eq:chirp_range}.}
    \label{fig:fmcw}
\end{figure}
...
In a radio system with more than one receive antenna, called an array, beamforming can be applied.
...
By applying a phase delay to the individual receive channels, the combined receiver array pattern can be manipulated to electronically point the direction from which the array is receiving.
...
FPGAs accelerate the signal processing allowing SDRs to be highly configurable.
They connect to computers using a USB or ethernet adapter. % TODO figure for SDR layout?
SDRs have become a very versatile tool for a variety of RF applications.
